...
try:
    with open('KFTSSB_config.json') as f:
        config = json.load(f)
...
STEAM_API_URL = config.get('steam_api_url')
...
async def get_steam_profiles(steam_ids, retry_count=1, retry_delay=15):
    ...
    async with aiohttp.ClientSession() as session:
        for attempt in range(retry_count + 1):
            try:
                response = await session.get(STEAM_API_URL, params=params)
                if response.status == 200:
                    data = await response.json()
                    for player in data['response']['players']:
                        sid = player['steamid']
                        name = player.get('personaname', 'Unknown User')
                        steam_profile_cache[sid] = name
                    break
                ...
                break
    # Return names for all requested IDs (from cache)
    return {sid: steam_profile_cache.get(sid, sid) for sid in steam_ids}
        ...
        for sid, info in session_payloads.items():
            ...
            if match_state != -1 and session_id not in active_embeds:
                ...
                if info['player_list']:
                    ...
                    player_list_str = "\n".join([profiles[sid] for sid in info['player_list']])
                else:
                ...
                active_embeds[session_id] = {'msg': msg, 'last_update': now}
...
async def tcp_listener(channel):
    ...
    try:
        ...
        print(f"Listening for TurboInfo packets on 127.0.0.1:{PORT}...")
    except Exception as e:
    ...
    while True:
        try:
            ...
            loop.create_task(handle_client(client, channel))
        except Exception as e:
            print(f"Error accepting connection: {e}")
            await asyncio.sleep(2)  # Prevent tight loop on repeated errors

async def handle_client(client, channel):
    buffer = ""
    loop = asyncio.get_event_loop()
    while True:
        try:
